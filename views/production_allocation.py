...
def render():
        ...
        for i,(key,qty) in enumerate(machine_list):
            ...
            with col:
                ...
                st.markdown(
                    ...
                    f'<div style="font-size:0.7rem;font-weight:700;color:#071952;">{m.get("name","—")}{"" if qty==1 else f" ×{qty}"}</div>'
                    f'<div style="font-size:0.72rem;color:#071952;">{fmt_rp(ct)}</div>'
                    f'<a href="{m.get("url","#")}" target="_blank" style="font-size:0.62rem;color:#071952;">Detail ↗</a>'
    ...
    for col,val,lbl,clr in [
        (c1,f"{len(products)} SKU","Total SKU","#071952"),
    ...
    colors_by_type={"SSS+BIB":"#071952","SSS":"#3fb950","STICKPACK":"#d29922"}
    ...
    with cl:
        ...
        fig_u.add_trace(go.Bar(x=all_lnames,y=[chart_new_util[l["name"]] for l in chart_lines],name="Setelah",
            marker_color=[colors_by_type.get(l["type"],"#071952") for l in chart_lines]))
    ...
    with _vv:
        ...
        if total_capex>0 and fin.get("cash_flows"):
            ...
            fig_cf.add_scatter(x=yrs,y=[c/1e6 for c in cum],mode="lines+markers",
                name="Kumulatif",line=dict(color="#071952",width=2))
            fig_cf.add_hline(y=0,line_dash="dash",line_color="#8b949e")
            fig_cf.update_layout(height=220,plot_bgcolor="#FFFFFF",paper_bgcolor="#FFFFFF",
    ...
    if invest_items:
        ...
        for _ri,(inv_label,cd,vol_ann,reason) in enumerate(invest_items):
            ...
            if "Stickpack" in inv_label or "STICKPACK" in inv_label:
                ...
                _sp_chip=(f'<span style="background:#FFFFFF;border:1px solid #088395;border-radius:4px;'                       f'padding:4px 14px;font-size:.82rem;color:#071952;margin:2px;">'                       f'Lini Stickpack (Baru): {_sp_d}D/{_sp_h}H</span>')
            _chips="".join(
                f'<span style="display:inline-block;background:#EBF4F6;border:1px solid #37B7C3;'             f'border-radius:6px;padding:4px 12px;margin:4px 3px;font-size:0.82rem;color:#071952;">{c}</span>'
                for c in _comps_rc)
            # Line config chips
            _lini_chips="".join(
                f'<span style="background:#EBF4F6;border:1px solid #37B7C3;border-radius:4px;'             f'padding:4px 14px;font-size:.82rem;color:#071952;margin:2px;">'             f'{l["name"]}: {l.get("days",7)}D/{l.get("hours",24)}H</span>'
                for l in lines[:5])
    ...
    <div class="dss-card" style="border:1px solid #37B7C3;border-radius:12px;padding:2rem;text-align:center;margin-bottom:1.5rem;">
      ...
      <div style="font-size:1.6rem;font-weight:800;color:#071952;margin:0.4rem 0;">{inv_label}</div>
