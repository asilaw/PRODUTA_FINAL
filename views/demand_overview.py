...
def _tab_forecasting():
    ...
    # ── B: Status Pipeline Forecast (konteks — ditampilkan jika ada raw data) ──
    sc_df = _load_sku_class()
    bt_df = _load_backtest_results()
    sk_df = _load_skipped_skus()

    if has_raw and (sc_df is not None or bt_df is not None):
        with st.expander("Status Pipeline Forecast (informasi database)", expanded=False):
            st.caption(
                "Ringkasan status pipeline forecast yang tersedia di sistem. "
                "Hanya SKU dengan status OK yang dimasukkan ke output forecast final."
            )
            # KPI pipeline
            n_classified = len(sc_df) if sc_df is not None else 0
            n_backtested = len(bt_df) if bt_df is not None else 0
            n_ok         = len(bt_df[bt_df["status"]=="OK"]) if bt_df is not None and "status" in bt_df.columns else 0
            n_review     = len(bt_df[bt_df["status"]=="REVIEW"]) if bt_df is not None and "status" in bt_df.columns else 0
            n_skipped    = len(sk_df) if sk_df is not None else 0

            pp1, pp2, pp3, pp4, pp5 = st.columns(5)
            for col_obj, lbl, val, note in [
                (pp1, "DIKLASIFIKASIKAN", str(n_classified), "dari raw data"),
                (pp2, "DIBACKTEST",       str(n_backtested), "berhasil diproses"),
                (pp3, "STATUS OK",        str(n_ok),         "masuk forecast"),
                (pp4, "STATUS REVIEW",    str(n_review),     "akurasi rendah"),
                (pp5, "DILEWATI",         str(n_skipped),    "data kurang"),
            ]:
                col_obj.markdown(
                    f'<div class="kpi-box">'
                    f'<div class="kpi-label">{lbl}</div>'
                    f'<div class="kpi-value">{val} SKU</div>'
                    f'<div style="font-size:.7rem;color:#088395;margin-top:3px;">{note}</div>'
                    f'</div>', unsafe_allow_html=True,
                )

            if bt_df is not None and "status" in bt_df.columns:
                # Tampilkan SKU yang REVIEW (perlu diperhatikan)
                review_skus = bt_df[bt_df["status"]=="REVIEW"][["sku","demand_pattern","model_used","wmape_backtest","mape_backtest"]]
                if not review_skus.empty:
                    st.markdown(
                        '<div class="warn-box">SKU berikut memiliki akurasi rendah dan belum masuk output forecast. Direkomendasikan verifikasi manual sebelum digunakan.</div>',
                        unsafe_allow_html=True,
                    )
                    for col in ["wmape_backtest","mape_backtest"]:
                        if col in review_skus.columns:
                            review_skus[col] = review_skus[col].round(1)
                    st.dataframe(_rl(review_skus), use_container_width=True, hide_index=True)
    ...
    if has_wmape and "sku" in fc_df.columns:
